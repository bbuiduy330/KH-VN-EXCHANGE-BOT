...
add("order.cancelled_by_admin_payment_note", "ℹ️ Khoản thanh toán của bạn đã được ghi nhận. Bộ phận chăm sóc khách sẽ xử lý hoàn trả/bước tiếp theo cho bạn.", "ℹ️ Your payment has been recorded. Our support team will handle the refund/next step for you.", "ℹ️ ការទូទាត់របស់អ្នកត្រូវបានកត់ត្រា។ ក្រុមគាំទ្រនឹងដោះស្រាយការបង្វិលសង / ជំហានបន្ទាប់សម្រាប់អ្នក។", "ℹ️ 您的付款已被记录。客服团队将为您处理退款/后续步骤。");
add("menu.marketing", "🔔 Thông báo ưu đãi", "🔔 Promotional messages", "🔔 ការជូនដំណឹងផ្សព្វផ្សាយ", "🔔 促销通知");
add("marketing.toggled_on", "🔔 Đã BẬT Thông báo ưu đãi. Bạn sẽ nhận được thông báo khuyến mãi/tỷ giá từ chúng tôi.\n\n(Thông báo về đơn hàng/thanh toán luôn được giữ nguyên.)", "🔔 Promotional messages are now ON. You will receive promo/rate updates from us.\n\n(Order/payment notifications are always unaffected.)", "🔔 ការជូនដំណឹងផ្សព្វផ្សាយបានបើក។ អ្នកនឹងទទួលបានព័ត៌មានតម្លៃ / ឱកាសពិសេស។\n\n(ការជូនដំណឹងអំពីការបញ្ជាទិញ / ការទូទាត់មិនត្រូវបានប៉ះពាល់ទេ។)", "🔔 促销通知已开启。您将收到我们的优惠/汇率信息。\n\n（订单/付款通知不受影响。）");
add("marketing.toggled_off", "🔕 Đã TẮT Thông báo ưu đãi. Bạn sẽ không nhận tin nhắn khuyến mãi nữa.\n\n(Thông báo về đơn hàng, thanh toán, hỗ trợ vẫn được gửi bình thường.)", "🔕 Promotional messages are now OFF. You will no longer receive promo messages.\n\n(Order, payment and support messages are still delivered normally.)", "🔕 ការជូនដំណឹងផ្សព្វផ្សាយបានបិទ។ អ្នកនឹងមិនទទួលសារផ្សព្វផ្សាយទៀតទេ។\n\n(សារអំពីការបញ្ជាទិញ, ការទូទាត់ និងការគាំទ្រនៅតែត្រូវបានផ្ញើដូច្នេះ។)", "🔕 促销通知已关闭。您将不再收到促销信息。\n\n（订单、付款和客服消息仍正常发送。）");
...
add("bill.pending_kept", "📎 Đã giữ ảnh bill của bạn. Vui lòng chọn đơn bên dưới để đính kèm — không cần gửi lại ảnh.", "📎 Your bill photo is saved. Choose the order below to attach it — no need to re-send the photo.", "📎 រូបភាពបង្កាន់ដៃរបស់អ្នកត្រូវបានរក្សាទុក។ សូមជ្រើសរើសការបញ្ជាទិញខាងក្រោម — មិនចាំបាច់ផ្ញើរូបភាពម្តងទៀតទេ។", "📎 已保存您的账单照片。请在下方选择订单进行附上——无需重新发送图片。");
add("bill.session_expired", "⏳ Phiên chọn đơn đã hết hạn hoặc ảnh không còn hiệu lực. Vui lòng gửi lại ảnh bill.", "⏳ The order-selection session expired or the photo is no longer available. Please send the bill photo again.", "⏳ សេស្សនជ្រើសរើសការបញ្ជាទិញផុតកំណត់ ឬរូបភាពមិនមានទៀតទេ។ សូមផ្ញើរូបភាពបង្កាន់ដៃម្តងទៀត។", "⏳ 选择订单的会话已过期或图片已失效。请重新发送账单图片。");
add("bill.processing", "⏳ Đang xử lý bill của bạn — vui lòng đợi trong giây lát.", "⏳ Processing your bill — please wait a moment.", "⏳ កំពុងដំណើរការបង្កាន់ដៃរបស់អ្នក — សូមរង់ចាំបន្តិច។", "⏳ 正在处理您的回单——请稍候。");
add("bill.retry_kept", "⚠️ Tạm thời chưa lưu được bill (lỗi hệ thống). Ảnh gốc vẫn được giữ — hãy bấm chọn lại đơn, không cần gửi lại ảnh.", "⚠️ The bill could not be saved yet (temporary error). Your original photo is kept — tap the order again, no need to re-send it.", "⚠️ មិនអាចរក្សាទុកបង្កាន់ដៃបានទេ (កំហុសបណ្តោះអាសន្ន)។ រូបភាពដើមរបស់អ្នកត្រូវបានរក្សាទុក — សូមចុចជ្រើសរើសការបញ្ជាទិញម្តងទៀត មិនចាំបាច់ផ្ញើរូបភាពម្តងទៀតទេ។", "⚠️ 暂时无法保存回单（临时错误）。您的原图已保留——请再次点击选择订单，无需重新发送图片。");
// Deterministic order-status replies (NEVER mention any system receiving
// account such as KHQR UPAY/BIDV — the payout destination belongs to the
// CUSTOMER and is collected after verification; the AI never builds these).
add("order.status_reply_wait_pay", "Dạ em cảm ơn {name} ạ! Đơn hàng <code>{id}</code> của quý khách đang chờ chuyển khoản. Sau khi chuyển tiền, anh/chị vui lòng gửi ảnh biên lai để nhân viên đối soát ạ.", "Thank you, {name}! Your order <code>{id}</code> is awaiting your bank transfer. After transferring, please send your payment receipt so our staff can verify it.", "សូមអរគុណ {name}! ការបញ្ជាទិញ <code>{id}</code> របស់អ្នកកំពុងរង់ចាំការផ្ទេរប្រាក់។ បន្ទាប់ពីផ្ទេរ សូមផ្ញើរូបភាពបង្កាន់ដៃដើម្បីឱ្យបុគ្គលិកពិនិត្យ។", "谢谢您，{name}！您的订单 <code>{id}</code> 正在等待您转账。转账后请发送付款凭证，工作人员将进行核对。");
add("order.status_reply_verifying", "Dạ em cảm ơn {name} ạ! Hệ thống đã ghi nhận biên lai của đơn hàng <code>{id}</code> và nhân viên đang tiến hành kiểm tra, đối soát giao dịch. Ngay sau khi xác nhận thanh toán thành công, hệ thống sẽ thực hiện chuyển tiền ngay về TK của quý khách trong ít phút ạ!", "Thank you, {name}! We have received the receipt for order <code>{id}</code> and our staff is verifying the transaction. As soon as the payment is confirmed, we will transfer the funds to YOUR account within minutes!", "សូមអរគុណ {name}! យើងបានទទួលបង្កាន់ដៃសម្រាប់ការបញ្ជាទិញ <code>{id}</code> ហើយបុគ្គលិកកំពុងពិនិត្យ។ ពេលបញ្ជាក់ទូទាត់ជោគជ័យ យើងនឹងផ្ទេរប្រាក់ទៅគណនីរបស់អ្នកក្នុងរយៈពេលបន្តិចទៀត!", "谢谢您，{name}！我们已收到订单 <code>{id}</code> 的付款凭证，工作人员正在核对交易。付款确认后，我们会立即将款项转入您的账户！");
add("order.status_reply_confirmed", "Dạ em cảm ơn {name} ạ! Hệ thống đã xác nhận thanh toán thành công cho đơn hàng <code>{id}</code>. Anh/chị vui lòng cung cấp tài khoản nhận tiền theo hướng dẫn để hệ thống thực hiện chuyển tiền ạ!", "Thank you, {name}! Payment for order <code>{id}</code> is confirmed. Please provide your receiving account so we can transfer the funds.", "សូមអរគុណ {name}! ការបង់ប្រាក់សម្រាប់ការបញ្ជាទិញ <code>{id}</code> ត្រូវបានបញ្ជាក់។ សូមផ្តល់គណនីទទួលរបស់អ្នកដើម្បីឱ្យយើងផ្ទេរប្រាក់!", "谢谢您，{name}！订单 <code>{id}</code> 的付款已确认。请提供您的收款账户，我们将尽快转账！");
add("order.status_reply_payout_info", "Dạ em cảm ơn {name} ạ! Hệ thống đã xác nhận thanh toán cho đơn hàng <code>{id}</code> và đang chờ quý khách chọn/cung cấp tài khoản nhận tiền. Sau khi có tài khoản nhận, hệ thống sẽ chuyển tiền ngay ạ!", "Thank you, {name}! Payment for order <code>{id}</code> is verified — we are waiting for you to choose/provide your receiving account. Once received, the transfer happens right away!", "សូមអរគុណ {name}! ការទូទាត់សម្រាប់ការបញ្ជាទិញ <code>{id}</code> ត្រូវបានផ្ទៀងផ្ទាត់ — សូមជ្រើសរើស/ផ្តល់គណនីទទួលរបស់អ្នក ហើយយើងនឹងផ្ទេរប្រាក់ភ្លាមៗ!", "谢谢您，{name}！订单 <code>{id}</code> 的付款已核实，正在等待您选择/提供收款账户。收到账户后我们会立即转账！");
add("order.status_reply_payout_sent", "Dạ em cảm ơn {name} ạ! Hệ thống đã thực hiện chuyển tiền cho đơn hàng <code>{id}</code>. Hóa đơn chuyển tiền đã được gửi đến quý khách — vui lòng kiểm tra tài khoản ạ!", "Thank you, {name}! The transfer for order <code>{id}</code> has been sent. The transfer receipt was delivered to you — please check your account!", "សូមអរគុណ {name}! យើងបានផ្ទេរប្រាក់សម្រាប់ការបញ្ជាទិញ <code>{id}</code>។ វិក្កយបត្រផ្ទេរប្រាក់ត្រូវបានផ្ញើរួច — សូមពិនិត្យគណនីរបស់អ្នក!", "谢谢您，{name}！订单 <code>{id}</code> 已完成转账，转账凭证已发送给您——请查收账户！");
